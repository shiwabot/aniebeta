...
from SaitamaRobot import (ASSE_USERS, DEV_USERS, OWNER_ID, SUDO_USERS, SUPPORT_CHAT,
...
@run_async
@asse_plus
def adsudo(update: Update, context: CallbackContext) -> str:
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    bot, args = context.bot, context.args
    ...
    if reply:
        ...
        return ""
    if HEROKU:
        config["SUDO_USERS"] = os.environ.get("SUDO_USERS") + " " + str(user_id)
        message.reply_text("Added Sexfully Now Restarting..")
    else:
        with open(ELEVATED_USERS_FILE, 'r') as infile:
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if HEROKU:
        str(user_id)
        ENF = os.environ.get("SUDO_USERS")
        if ENF.endswith(str(user_id)):
            config["SUDO_USERS"] = ENF.strip(" " + str(user_id))
        elif ENF.startswith(str(user_id)):
            config["SUDO_USERS"] = ENF.strip(str(user_id) + " ")
        else:
            config["SUDO_USERS"] = ENF.strip(" " + str(user_id) + " ")
            message.reply_text("Sexfully Removed Now Restarting...")
    else:
        with open(ELEVATED_USERS_FILE, 'r') as infile:
            data = json.load(infile)
...
@run_async
@sudo_plus
def asselist(update: Update, context: CallbackContext):
    bot = context.bot
    true_asse = list(set(ASSE_USERS) - {OWNER_ID})
    reply = "<b>THE ASSEMBLER⚡️:</b>\n"
    for each_user in true_asse:
        user_id = int(each_user)
        try:
            user = bot.get_chat(user_id)
            reply += f"• {mention_html(user_id, user.first_name)}\n"
...
ADSUDO_HANDLER = CommandHandler(("adsudo", "adddragon"), adsudo)
...
ASSELIST_HANDLER = CommandHandler(["assembler", "asseuser"], asselist)
...
dispatcher.add_handler(ASSELIST_HANDLER)
