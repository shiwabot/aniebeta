...
from SaitamaRobot.modules.helper_funcs.chat_status import user_admin_no_reply, user_admin

from SaitamaRobot.modules.sql import languages_sql as sql
...
for x in os.listdir('SaitamaRobot/modules/langs'):
	if os.path.isdir('SaitamaRobot/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("SaitamaRobot.modules.langs." + x)
...
def set_language(bot, update):
	...
	send_message(update.effective_message, tl(msg, "Language in *{}* now is:\n{}.\n\nChoose language:").format(chatname, LANGS_TEXT[getlang]), parse_mode="markdown", reply_markup=keyboard)
