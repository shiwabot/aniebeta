...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Kicked! {mention_html(member.user.id, member.user.first_name)}.",
            parse_mode=ParseMode.HTML)
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#KICKED\n"
...
def punchme(update: Update, context: CallbackContext):
    ...
    if res:
        update.effective_message.reply_text("*Get you out of the group*")
    else:
...
*Admins only:*
 ...
 • `/dban <username>`*:* ban a user and delete his message both is easy now (vai handle or reply)
"""
...
__mod_name__ = "Bans"
__handlers__ = [
    BAN_HANDLER, DBAN_HANDLER, TEMPBAN_HANDLER, PUNCH_HANDLER, UNBAN_HANDLER, ROAR_HANDLER,
    PUNCHME_HANDLER
]
